...
LRESULT CGameFrameWork_D2D::WndProc(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SOCKET:
		CIOCPClient::GetInstance()->SocketProc(hWnd, nMessageID, wParam, lParam);
		break;
...
void CGameFrameWork_D2D::Update(const float& fTime)
{
	...
	if (Input->OnlyKeyDown(YK_Q)) SendMessage(m_hWnd, WM_DESTROY, NULL, NULL);
}
...
void CGameFrameWork_D2D::Render()
{
	...
	Draw::drawText
	(
		 m_hWndRenderTarget.Get()
		, RectF(150, 200, 500, 230)
		, FONT_FORMAT::FONT_나눔바른고딕
		, MY_COLOR(MyColorEnum::Azure)
		, TEXT("( %d, %d ) ")
		, pt.x, pt.y
