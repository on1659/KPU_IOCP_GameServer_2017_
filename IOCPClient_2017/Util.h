...
public:
	...
	void Set(const D2D_RECT_F& rt)
	{
		rect = rt;
		SetBoxLTRD(rect.left, rect.top, rect.right, rect.bottom);
	}
	...
	void OffSetSize(const float& x, const float& y) { m_size.x += x; m_size.y += y; update(); }
	void OffSetSize(const Vector2& v2)		    { OffSetSize(v2.x, v2.y); }
	...
	const float& GetLeft() const { return rect.left; }
	const float& GetTop() const { return rect.top; }
...
inline static Vector2 operator-(const Vector2& a, const D2D_RECT_F& b)
{
	return Vector2{a.x - b.left, a.y - b.top};
};
inline static Vector2 operator-(const D2D_RECT_F& b, const Vector2& a)
{
	return Vector2{ a.x - b.left, a.y - b.top };
};
