...
public:
	...
	void Set(const D2D_RECT_F& rect) { m_bcBoundingBox.Set(rect); }
	...
	virtual void OffSetSize(const Vector2& v2) { m_bcBoundingBox.OffSetSize(v2); }
	virtual void OffSetSize(const float& x, const float& y) { m_bcBoundingBox.OffSetSize(x,y); }
	virtual void OffSetSize(const float& offset) { m_bcBoundingBox.OffSetSize(offset, offset);}
	...
	const float GetLeft() const { return m_bcBoundingBox.GetLeft(); }
	const float GetTop() const  { return m_bcBoundingBox.GetTop(); }
