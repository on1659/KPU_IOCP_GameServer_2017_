...
void CGameObject_2D::Render(ID2D1HwndRenderTarget * pd2dRenderTarget, CCamera_2D * pCamera)
{
	for (auto& sprite : m_vSprite)
		sprite.SetBoundingBox(m_bcBoundingBox);
...
void CGameObject_2D::Render(ID2D1HwndRenderTarget * pd2dRenderTarget, const D2D_RECT_F rtCamera)
{
	for (auto& sprite : m_vSprite)
		sprite.SetBoundingBox(m_bcBoundingBox);
